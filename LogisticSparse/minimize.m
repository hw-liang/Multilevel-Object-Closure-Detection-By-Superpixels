function [X, fX, i,Xs] = minimize(X, f, length, varargin)
...
SIG = 0.2; RHO = SIG/2; % SIG and RHO are the constants controlling the Wolfe-
...
%   outputFunc(X, varargin);
  ...
  if abs(d3) < -SIG*d0 && f3 < f0+x3*RHO*d0          % if line search succeeded
    X = X+x3*s; f0 = f3; fX = [fX' f0]'; Xs = [Xs, X];                     % update variables
%     fprintf('%s %6i;  Value %4.6e\r', S, i, f0);
    ...
    X = X0; f0 = F0; df0 = dF0;  Xs = [Xs, X]; % restore best point so far
...
fprintf('Optimization terminated after %6i iterations:  Value %4.6e\r', i, f0);
% fprintf('\n');

function outputFunc(x, varargin)    
    disp(['Current params: ', num2str(x')]);
