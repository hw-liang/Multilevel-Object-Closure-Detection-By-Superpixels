...
if (strcmp(mexext, 'mexglx')) % 32 bit system
    system(sprintf('./lib/segment32 -image %s -smatrixfile %s -readlattice true -latticefile %s -dthresh 5', imgFile, simFile, latFile));
elseif (strcmp(mexext, 'mexa64')) % 64 bit system
    system(sprintf('./lib/segment64 -image %s -smatrixfile %s -readlattice true -latticefile %s -dthresh 5', imgFile, simFile, latFile));
else
    error('BSE segment program not supported on this platform');
end
