...
while (ido ~= 99)
    ...
    if isrealprob
        ...
        [ido, info] = arpackc( aupdfun, ido, bmat, intconvert(n), whch, ...
            nev, tol, resid, ncv, zv, ldv, iparam, ipntr, zworkd, workl, ...
            lworkl, rwork, info );
...
if isrealprob
    if issymA
        ...
        if strcmp(whch,'LM') || strcmp(whch,'LA')
            d(converged) = flipud(d(converged));
            if (rvec == 1)
                v(:,converged) = fliplr(v(:,converged));
            end
        end
...
%-------------------------------------------------------------------------%
    function printTimings
        ...
        if (mode == 1)
            innerstr = getString(message('MATLAB:eigs:PrintTimingsComputeAX',sprintf('%f',cputms(3))));
        elseif (mode == 3)
            if isempty(B)
