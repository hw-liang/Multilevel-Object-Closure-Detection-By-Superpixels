...
function [X, Xs, cost, sup_image, costs] = SuperpixelClosureGrouping(img_filename, ...
    ...
    if (exist('selected_sups', 'var') && ~isempty(selected_sups))
        ...
        num_sups = sum(selected_sups);
        adjmat = adjmat(selected_sups, selected_sups);  % make a adjacent matrix only for the selected superpixels
        sup_image(SupValueImage_MEX(sup_image, double(selected_sups)) == 0) = 0;  % make the selected pixels unselected in the label image
    ...
    D = sum(affinity,2);  % a column with sums of rows
%     X = MinimizeRatioBinary(double(rand(num_sups,1)>0.5),G, affinity);
    ...
    if (recursive_on)
        if (num_solutions == 1)
            ...
            Xs1 = [];
            if (sum(X > 0.5) > 1)
                [X1, Xs1, cost1, sup_image1] = SuperpixelClosureGrouping(img_filename, sup_image, image_data, num_solutions-1, X > 0.5);
                repetitions = sum((Xs1 == repmat(X, 1, size(Xs1, 2))), 1) == numel(X);
                Xs1 = Xs1(:, ~repetitions);
            end
            Xs2 = [];
            if (sum(X < 0.5) > 1)
